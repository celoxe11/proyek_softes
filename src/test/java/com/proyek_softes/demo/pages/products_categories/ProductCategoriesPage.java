...
public class ProductCategoriesPage {
    ...
    private final By subTab = By.xpath(
            "//a[contains(@href, 'module=AOS_Product_Categories')]");
     ...
     * =========================
     */
    private final By firstRowProductName = By.cssSelector("table.list.view tbody tr:first-child td[type='name'] a");
    ...
    public void navigateToProductsModule() {
        ...
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(subTab)).click();
        } catch (org.openqa.selenium.TimeoutException e) {
            // Fallback: If hover doesn't make it visible, use JS click
            WebElement element = wait.until(ExpectedConditions.presenceOfElementLocated(subTab));
    ...
    public boolean isInFirstRow(String productName) {
        try {
            ...
            String firstRowName = driver.findElement(firstRowProductName).getText().trim();
    ...
    public boolean isProductTitleCorrect(String productName) {
        try {
            String title = wait.until(ExpectedConditions.visibilityOfElementLocated(pageTitle))
                    .getText();
    ...
    public void checkAndClearFilter() {
        ...
        boolean hasFilter = !nameValue.isEmpty()
