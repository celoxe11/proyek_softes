...
public class CasesPage {
    ...
    // Navbar untuk navigasi ke Cases
    private final By navTab = By.id("grouptab_5");
    private final By subTab = By.xpath("//a[@href=\"index.php?module=Cases&action=index&parentTab=All\"]");

    private final By createCaseLink = By.xpath("//a[@data-action-name='Create']");
    private final By viewCaseLink = By.xpath("//a[@data-action-name='List']");
    private final By importCaseLink = By.xpath("//a[@data-action-name='Import']");

    private final By firstRowCaseName = By.cssSelector("table.list.view tbody tr:first-child td[type='name'] a");
    ...
    public void navigateToCasesModule() {
    ...
    public void navigateToCreateCase() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(createCaseLink));
        wait.until(ExpectedConditions.elementToBeClickable(createCaseLink));
        driver.findElement(createCaseLink).click();
    }

    public void navigateToViewCase() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(viewCaseLink));
        wait.until(ExpectedConditions.elementToBeClickable(viewCaseLink));
        driver.findElement(viewCaseLink).click();
    }

    public void navigateToImportCase() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(importCaseLink));
        wait.until(ExpectedConditions.elementToBeClickable(importCaseLink));
        driver.findElement(importCaseLink).click();
    }

    public boolean isInFirstRow(String caseName) {
        try {
            ...
            String firstRowCaseName = driver.findElement(firstRowNameLocator).getText().trim();
            System.out.println(firstRowCaseName);
            return firstRowCaseName.contains(caseName);
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public WebElement getFirstRowNameLocator() {
        wait.until(ExpectedConditions.presenceOfElementLocated(firstRowCaseName));
        return driver.findElement(firstRowCaseName);
    }
    ...
    public boolean isCaseTitleCorrect(String caseName) {
        try {
