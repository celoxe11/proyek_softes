...
public class NotesPage {
    ...
    // Navbar untuk navigasi ke Notes
    private final By navTab = By.id("grouptab_5");
    private final By subTab = By.xpath("//a[@href=\"index.php?module=Notes&action=index&parentTab=All\"]");

    private final By createNoteLink = By.xpath("//a[@data-action-name='Create']");
    ...
    public void navigateToCreateNote() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(createNoteLink));
        wait.until(ExpectedConditions.elementToBeClickable(createNoteLink));
        driver.findElement(createNoteLink).click();
    }

    public void navigateToViewNote() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(viewNoteLink));
        wait.until(ExpectedConditions.elementToBeClickable(viewNoteLink));
        driver.findElement(viewNoteLink).click();
    }

    public void navigateToImportNote() {
        wait.until(ExpectedConditions.visibilityOfElementLocated(importNoteLink));
        wait.until(ExpectedConditions.elementToBeClickable(importNoteLink));
        driver.findElement(importNoteLink).click();
    }
    ...
    public boolean isInFirstRow(String noteName) {
        try {
            ...
            return firstRowText.contains(noteName);
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public WebElement getFirstRowNameLocator() {
        wait.until(ExpectedConditions.presenceOfElementLocated(firstRowNoteName));
        return driver.findElement(firstRowNoteName);
    }
    ...
    public boolean isNoteTitleCorrect(String noteName) {
        try {
